...
import { getFollowListAPI, type FollowListParams } from '@/services/index/page'
...
const fetchFollowListData = async (page: number): Promise<HomeItem[]> => {
  try {
    ...
    const res = await getFollowListAPI(params)
    ...
    console.log('获取关注列表数据==111111111111111===', res.data)
    // 返回列表数据
    return res.data.list || []
  } catch (error) {
    console.error('请求关注列表失败:=========', error)
    return []
  }
}
